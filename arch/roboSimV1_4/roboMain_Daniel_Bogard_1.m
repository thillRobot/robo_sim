...
% unknown environment
% Version 1.1 - S.C
% In this version, the robot consi1sts of a lumped mass with no preferred direction
...
thresh=9; %safe distance from object
sidethresh=5;
slightthresh=10;
...
%    program robot behaviors in this section    
    ...
    if ~avoiding;
        ...
        if sen_dist(5)<thresh && sen_dist(3)<thresh && sen_dist(6)<thresh && sen_dist(2)<thresh
            disp('b10')
            gas=-400;
            steer=2;
            avoiding=4 ;
        ...
        elseif sen_dist(3)<sen_dist(5) && sen_dist(4)<thresh
            disp('b2')
            gas=400;
            steer=-3;
            avoiding=1 ;
            
        elseif sen_dist(5)<sen_dist(3) && sen_dist(4)<thresh
            disp('b3')
            gas=400;
            steer=3;
            avoiding=1 ;
            
        elseif sen_dist(7)<sidethresh
            disp('b4')
            gas=400;
            steer=5;
            avoiding=1 ;  
            
        elseif sen_dist(1)<sidethresh
            disp('b5')
            gas=400;
            steer=-5;
            avoiding=1 ; 
            
        elseif sen_dist(4)<thresh % if danger is in front
            disp('b1')
            gas=-400;          % reverse
            steer=0;          % dont steer 
            avoiding=4 ;      % for five seconds (itehrations)
        end
    else
...
%     pause
    ...
    if escaped
        msg=sprintf('Congratulations, your robot escaped the arena in %f seconds!!!',toc);
        title(msg)
        fprintf(msg)
    elseif crashed
        msg=sprintf('Opps, your robot crashed and was destroyed in %f seconds!!!',toc);
        title(msg)
        fprintf(msg)
    end
